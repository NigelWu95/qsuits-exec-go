...
func AccountUsage() {
	...
	fmt.Println("-getaccount=s3-test 表示查询设置的 S3 账号名为 test 的密钥")
	fmt.Println("-getaccount=ten-test 表示查询设置的腾讯账号名为 test 的密钥")
	fmt.Println("-getaccount=qiniu-test 表示查询设置的七牛账号名为 test 的密钥")
	fmt.Println()
	fmt.Println("4. 删除 account 账号：")
	fmt.Println("命令格式：-delaccount=<name>-<source>，删除账号只允许一次删除一条，如：")
	fmt.Println()
	fmt.Println("-delaccount=s3-test 表示删除设置的 S3 账号名为 test 的密钥")
	fmt.Println("-delaccount=ten-test 表示删除设置的腾讯账号名为 test 的密钥")
	fmt.Println("-delaccount=test/qiniu-test 表示删除设置的七牛账号名为 test 的密钥")
}
