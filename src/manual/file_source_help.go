...
func FileUsage() {
	...
	fmt.Println("line-config     配置文件路径，表示从该配置中读取文件名作为 file 数据源，同时文件名对应的值表示读取该文件的起始位置，配置文件格式为 json，参考 line-config 配置文件")

	fmt.Println()
	fmt.Println("数据源详细参数配置和说明及可能涉及的高级用法见：数据源配置 https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/datasource.md")
}
